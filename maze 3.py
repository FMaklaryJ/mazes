...
start=10
victory=20 #must be >start+2

maze[ec[0,0],ec[0,1]]=victory
maze[X0[0,0],X0[0,1]]=start
realmaze=maze.copy()

print(maze)

#-----Rounds, intial conditions, and boolian, global variables:
...
for r in range(rounds):
    ...
    while J==1 and steps<step0:
        ...
        #Dead ends
            ...
            maze[L[1],L[0]]=1
            J=0
        #ending at spawn (same as dead end, but don't block off)
        elif K>=start and K<=victory:
            fails+=1
            Xfails=np.append(Xfails,X,axis=1)
            v0=-v0
            X=X+v0
            ...
            J=0
        steps+=1
    #Updating shortest path to goal:
    if steps<step0 and win==1:
        step0=steps
        p0=p.copy()
        Xfails0=Xfails.copy()
        fails0=fails
...
realmazecords=np.argwhere(realmaze==0)
        
            

print("done")
plt.plot(realmazecords[:,1],-realmazecords[:,0],'b.')
plt.plot(mazecords[:,1],-mazecords[:,0],'k.')
plt.title("Couldn't solve it, sorry.")
if WIN==1:
    p0[1,:]=-p0[1,:]
    plt.plot(p0[0,:],p0[1,:],'-')
